...
TWIR_calc = {
	calcBest: function(skills, para, callback) {
		var sets = TWIR_calc.getBestSet(skills, para);
		TWIR_calc.result = sets;
		callback(sets);
		return sets;
	},
	...
	getBestSet: function(skills, para) {
		...
		bestItems = TWIR_calc.getBestItems(skills, para); // 3.
		...
		sets = TWIR_calc.createSubsets(availableSets, bestItems, skills, para); // 4.
		if (window.__limitclothcalc && sets.length > 500) {
			sets = TWIR_calc.createSubsets(availableSets, bestItems, skills, para, true);
			console && console.log('using approximation...');
		}
		sets = TWIR_calc.filterUneffectiveSets(sets, skills, para); //5.
		sets = west.item.Calculator.fillEmptySlots(west.item.Calculator.combineSets(sets), bestItems);
		sets.push(bestItemsContainer);
		for (i = 0; i < sets.length; i++) {
			sets[i] = TWIR_calc.itemsToSets(sets[i]);
			switch (para) {
				case "ms":
					var tmpArr = TWIR_calc.getValue4Container(skills, sets[i], para);
					tmp = TWIR_calc.calcSpeed([tmpArr[0], tmpArr[1]]);
					break;
				default:
					tmp = TWIR_calc.getValue4Container(skills, sets[i], para);
					break;
			}
			sets[i]["tmp"] = tmp;
			if (tmp > points) {
	...
	getPointAddition: function(bonus, _valueModifier, _roundingMethod) {
		if (bonus.type == 'character' && bonus.key == 'level') {
			return TWIR_calc.getPointAddition(bonus.bonus, function(val) {
				return TWIR_calc.getRoundedValue(val * Character.level, bonus.roundingMethod);
	...
	getValue: function(skills, item, para, object) {
		...
		if (item.hasItemBonus()) {
			...
			for (i = 0; i < item.bonus.item.length; i++) {
				...
				var specialSkills = TWIR_calc.getAdditionalBonus(item.bonus.item[i], item.getItemLevel());
				...
				if (!object) value += TWIR_calc.getPointAddition(item.bonus.item[i]); //potrebujem to???
			}
		}
		//dmg zbrane
		...
		for (skill in skills) {
			...
			//if (object) value[skill] = (value[skill] || 0) + (skills[skill] * TWIR_calc.calcByFormula(skill, bonuses, para));
			//else value += skills[skill] * TWIR_calc.calcByFormula(skill, bonuses, para);
			//speed zo zvierata?
	...
	getValue4Set: function(skills, set, para, base_id) {
		...
		switch (para) {
			case "ms":
				var setValues = TWIR_calc.getSetValue({
					"speed": 1,
					"ride": 1
				}, set, para, true);
				...
				break;
			default:
				boni += TWIR_calc.getSetValue(skills, set, para);
				break;
		}
		...
		for (var i = 0; i < usedSetItems.length; i++) {
			switch (para) {
				case "ms":
					var itemValues = TWIR_calc.getValue({
						"speed": 1,
						"ride": 1
					}, ItemManager.get(usedSetItems[i]), para, true);
					speedBoni += (itemValues.speed || 0);
					rideBoni += (itemValues.ride || 0);
					if (i == (usedSetItems.length - 1)) return TWIR_calc.calcSpeed([rideBoni, speedBoni]); //end of array
					break;
				default:
					boni += TWIR_calc.getValue(skills, ItemManager.get(usedSetItems[i]), para);
	...
	getSetValue: function(skills, set, para, object) {
		...
		for (i = 0; i < mergedStages.length; i++) {
			...
			var specialSkills = TWIR_calc.getAdditionalBonus(mergedStages[i], null);
			for (skill in affectedSkills) {
				//exported bonuses
				if (!bonuses[skill]) bonuses[skill] = 0;
				bonuses[skill] += affectedSkills[skill];
			...
			if (!object) value += TWIR_calc.getPointAddition(mergedStages[i]);
		}
		for (skill in skills) {
			//if (object) value[skill] = (value[skill] || 0) + (skills[skill] * TWIR_calc.calcByFormula(skill, bonuses, para));
			//else value += skills[skill] * TWIR_calc.calcByFormula(skill, bonuses, para);
		...
		for (i = 0; i < items.length; i++) {
			switch (para) {
				case "ms":
					...
					break;
				default:
					boni += TWIR_calc.getValue(skills, ItemManager.get(items[i]), para);
					break;
			}
		}
		switch (para) {
			case "ms":
				var arrBoni = TWIR_calc.getSetValue4Container(skills, set, para);
				speedBoni += arrBoni[1];
				rideBoni += arrBoni[0];
				return [rideBoni, speedBoni];
				break;
			default:
				boni += TWIR_calc.getSetValue4Container(skills, set, para);
				return boni;
				break;
		}
	},
	getSetValue4Container: function(skills, container, para) {
		...
		for (i = 0; i < container.sets.length; i++) {
			switch (para) {
				case "ms":
					var getVal = TWIR_calc.getSetValue({
						"speed": 1,
						"ride": 1
					...
					break;
				default:
					boni += TWIR_calc.getSetValue(skills, container.sets[i], para);
	...
	getBestItems: function(skills, para) { //3
		...
		west.common.forEach(items_by_base, function(items, base_id) {
			...
			switch (para) {
				case "ms":
					var getItemVal = TWIR_calc.getValue({
						"speed": 1,
						"ride": 1
					}, item, para, true);
					...
					break;
				default:
					value = TWIR_calc.getValue(skills, item, para); //1
					break;
			}
		...
		west.common.forEach(best_items, function(items, type) {
			...
			if (wear_item) {
				...
				switch (para) {
					case "ms":
						var getWearVal = TWIR_calc.getValue({
							"speed": 1,
							"ride": 1
						}, wear_item, para, true);
						...
						break;
					default:
						wear_value = TWIR_calc.getValue(skills, wear_item, para); //2
						break;
				}
				items.push({
					item: wear_item,
		...
		for (i = 0; i < bestItems.length; i++) {
			...
			switch (para) {
				case "ms":
					...
					break;
				default:
					bestItemsPoints += TWIR_calc.getValue(skills, bestItems[i], para); //item value
					break;
			}
		}
		switch (para) {
			case "ms":
				bestItemsPoints = TWIR_calc.calcSpeed([rideBoni, speedBoni]);
				break;
		}
		return TWIR_calc.getValue4Set(skills, set, para) > bestItemsPoints; //set value
	},
	...
	createCombinations: function(items, k) {
		...
		for (i = 0; i < items.length - k + 1; i++) {
			...
			tailcombs = TWIR_calc.createCombinations(items.slice(i + 1), k - 1);
	...
	sortSetItems: function(skills, set, para) {
		...
		set.items.sort(function(a, b) {
			return TWIR_calc.getValue(skills, get(b), para) - TWIR_calc.getValue(skills, get(a), para);
		});
	},
	weapon: "shot", //UPRAVIT
	filterWeapons: function(items) { //len konkretna zbran
		...
		for (var a = 0; a < items.length; a++) {
			...
			if (getItem.type === "right_arm" && getItem.sub_type !== TWIR_calc.weapon) toDelete = items[a];
		}
	...
	createSubsets: function(fullSets, bestItems, skills, para, useApproximation) {
		...
		for (i = 0; i < fullSets.length; i++) {
			set = fullSets[i];
			if (useApproximation && para !== "ms") TWIR_calc.sortSetItems(skills, set, para);
			for (j = set.items.length; j > 0; j--) {
				if (!set.bonus.hasOwnProperty(j)) continue;
				var filtered = TWIR_calc.filterWeapons(set.items);
				if (useApproximation && para !== "ms") permutations = [set.items.slice(0, j)];
				else permutations = TWIR_calc.createCombinations(filtered, j);
				for (k = 0, l = permutations.length; k < l; k++) {
					if (!TWIR_calc.itemsCombineable(permutations[k])) continue;
					tmpSet = new west.item.ItemSet({
						key: set.key,
						items: permutations[k],
						bonus: set.bonus
					});
					if (!TWIR_calc.beatsBestItems(tmpSet, bestItems, skills, para)) continue;
					sets.push(tmpSet);
				}
			}
		}
		...
		for (i = 0; i < sets.length; i++) {
			switch (para) {
				case "ms":
					...
					speedBoni = (getVal.speed || 0);
					rideBoni = (getVal.ride || 0);
					setValue = TWIR_calc.calcSpeed([rideBoni, speedBoni]);
					break;
				default:
					setValue = TWIR_calc.getSetValue(skills, sets[i], para);
					break;
			}
			...
			if (!bestBySlots[slots]) {
				...
				value = TWIR_calc.getValue4Set(skills, sets[i], para);
				if (TWIR_calc.getValue4Set(skills, bestBySlots[slots], para) < value) bestBySlots[slots] = sets[i];
			}
		}
